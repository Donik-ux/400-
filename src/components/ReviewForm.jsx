...
export default function ReviewForm({ t, onSubmitted }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-[#eef2f5] rounded-2xl border border-[#dfe7ec] shadow-soft p-5">
      <div className="text-[13px] font-black text-[#252a31] mb-3">{t('homePage.reviews.formTitle')}</div>
      ...
      <div className="flex items-center gap-1 mb-3" onMouseLeave={() => setHoverRating(0)}>
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} type="button" onClick={() => setRating(n)} onMouseEnter={() => setHoverRating(n)}
            ...
            <Star className={`w-5 h-5 transition-colors ${(hoverRating || rating) >= n ? 'fill-[#00a58e] text-[#00a58e]' : 'text-[#bac7d1]'}`} />
          </button>
        ))}
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-2 mb-2">
        <input value={name} onChange={(e) => setName(e.target.value)} maxLength={60} required
          ...
          className="px-3 py-2.5 rounded-xl border border-[#dfe7ec] bg-white text-[13px] font-semibold text-[#252a31] placeholder:text-[#94a3af] outline-none focus:border-[#0172cb] transition" />
      ...
      <input value={destination} onChange={(e) => setDestination(e.target.value)} maxLength={60}
        ...
        className="w-full px-3 py-2.5 rounded-xl border border-[#dfe7ec] bg-white text-[13px] font-semibold text-[#252a31] placeholder:text-[#94a3af] outline-none focus:border-[#0172cb] transition mb-2" />
