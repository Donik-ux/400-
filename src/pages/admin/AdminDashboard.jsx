  ...
  return (
    <div className="min-h-screen night-bg flex">
        ...
        <nav className="flex-1 px-3 py-4 flex flex-col gap-1 overflow-y-auto">
          {TABS.map(tb => (
            <button key={tb.id} onClick={() => setTab(tb.id)}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left text-[12px] font-bold uppercase tracking-[0.1em] transition-premium ${tab === tb.id ? 'bg-gradient-to-r from-[#61d1bf] to-[#009882] text-[#14181d] shadow-soft' : 'text-white/40 hover:text-white hover:bg-white/[0.04]'}`}>
              <tb.icon className="w-4 h-4 flex-shrink-0" />
              <span className="flex-1">{tb.label}</span>
              {tb.badge > 0 && (
                <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-full ${tab === tb.id ? 'bg-[#14181d]/20 text-[#14181d]' : 'bg-[#00a58e]/15 text-[#61d1bf]'}`}>{tb.badge}</span>
              )}
            </button>
          ))}
        </nav>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-30 night-side border-t flex">
        {TABS.map(tb => (
          <button key={tb.id} onClick={() => setTab(tb.id)}
            className={`flex-1 py-3 flex flex-col items-center gap-1 text-[9px] font-black uppercase tracking-widest transition-premium ${tab === tb.id ? 'text-[#61d1bf]' : 'text-white/30'}`}>
            <div className="relative">
              <tb.icon className="w-5 h-5" />
              {tb.badge > 0 && <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-[#00a58e] text-[7px] font-black flex items-center justify-center text-[#14181d]">{tb.badge}</span>}
            </div>
            {tb.label.slice(0,4)}
          </button>
        ))}
          ...
          <div className="flex flex-wrap gap-2">
            {['week', 'month'].map(rangeOption => (
              <button key={rangeOption} type="button" onClick={() => setActivityRange(rangeOption)}
                className={`px-3 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition ${activityRange === rangeOption ? 'bg-[#00a58e] text-[#14181d]' : 'night-inset text-white/60 hover:bg-[#8fb4ff]/10'}`}>
              ...
              <div key={label} className="space-y-2">
                ...
                <div className="h-3 rounded-full bg-[#8fb4ff]/10 overflow-hidden">
                  <div style={{ width: `${(usageChart.counts[index] / maxValue) * 100}%` }}
                    className="h-full rounded-full bg-gradient-to-r from-[#007f6d] to-[#61d1bf] transition-all duration-300" />
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex justify-end">
        <button onClick={startAdd} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#61d1bf] to-[#009882] text-[#14181d] text-[11px] font-black uppercase tracking-widest transition-premium hover:brightness-110 shadow-soft">
