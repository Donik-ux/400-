...
export default function FlightCard({ flight, index, aiPriced }) {
  ...
  return (
    <>
      <div
        className="group relative bg-white border border-[#dfe7ec] hover:border-[#0172cb]/45 shadow-soft hover:shadow-lift rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 ease-out hover:-translate-y-0.5 page-fade"
      ...
      >
          ...
          <div className="flex-1 p-5 lg:p-6 flex flex-col sm:flex-row sm:items-center gap-5">
            ...
            {officialUrl ? (
              <a href={officialUrl} target="_blank" rel="noopener noreferrer"
                ...
                className="flex items-center gap-3 sm:w-44 shrink-0 rounded-xl -m-1.5 p-1.5 hover:bg-[#e8f4fd] transition-colors group/airline"
                title={`${t('flightsPage.card.bookDirectTitle')} ${meta?.domain || flight.airline}`}>
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#f3f8ff] to-[#d6ebfb] border border-[#d6ebfb] shadow-soft flex items-center justify-center text-2xl shrink-0 group-hover/airline:scale-105 group-hover/airline:shadow-float transition-all duration-300">
                  {meta?.flag || flight.airlineLogo || <Plane className="w-5 h-5 text-[#0172cb] -rotate-45" />}
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className="text-[14px] font-extrabold text-[#252a31] truncate tracking-tight">{flight.airline}</p>
                    <BadgeCheck className="w-3.5 h-3.5 text-[#008009] shrink-0" />
                  </div>
                  <p className="text-[11px] text-[#0172cb] font-bold truncate flex items-center gap-1 group-hover/airline:underline decoration-[#0172cb]/40 underline-offset-2">
              ...
              <div className="flex items-center gap-3 sm:w-44 shrink-0">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#f3f8ff] to-[#d6ebfb] border border-[#d6ebfb] shadow-soft flex items-center justify-center text-2xl shrink-0">
                  {flight.airlineLogo || <Plane className="w-5 h-5 text-[#0172cb] -rotate-45" />}
                </div>
                <div className="min-w-0">
                  <p className="text-[14px] font-extrabold text-[#252a31] truncate tracking-tight">{flight.airline}</p>
                  <p className="text-[11px] text-[#697d95] font-semibold">{flight.airlineCode} · {flight.cabin}</p>
                </div>
              </div>
            )}
            ...
            <div className="flex-1 flex items-center gap-3 sm:gap-4 w-full">
              <div className="text-center shrink-0 min-w-[64px]">
                <p className="text-[19px] font-extrabold text-[#252a31] leading-tight tracking-tight">{fromCity}</p>
                {flight.date && <p className="text-[10px] text-[#0172cb] font-bold mt-0.5 uppercase tracking-wide">{formatFlightDate(flight.date)}</p>}
              </div>
              ...
              <div className="flex-1 flex flex-col items-center gap-1.5 min-w-0 px-1">
                <div className="w-full flex items-center gap-1.5">
                  ...
                  <div className="w-2.5 h-2.5 rounded-full bg-[#252a31] shrink-0" />
                </div>
                <span className={`text-[10px] font-extrabold uppercase tracking-wider ${nonStop ? 'text-[#008009]' : 'text-[#007f6d]'}`}>
                  {nonStop
              ...
              <div className="text-center shrink-0 min-w-[64px]">
                <p className="text-[19px] font-extrabold text-[#252a31] leading-tight tracking-tight">{toCity}</p>
              </div>
            </div>
          </div>

          {/* ── Right: price + actions (tinted panel) ── */}
          <div className="lg:w-[218px] shrink-0 bg-gradient-to-b from-[#f5f7f9] to-[#eef2f5] border-t lg:border-t-0 lg:border-l border-[#e8edf1] p-5 lg:p-6 flex flex-row lg:flex-col items-center lg:items-stretch justify-between gap-4">
            <div className="text-left lg:text-center">
              <p className="text-[28px] font-extrabold text-gradient leading-none tabular-nums tracking-tight">
                {!isRealPrice && <span className="text-[#697d95] text-[20px] font-bold">≈ </span>}<Price amount={flight.price} />
              </p>
              <p className="text-[10.5px] text-[#697d95] font-bold mt-1.5">
                {isRealPrice
            ...
            <div className="flex flex-col items-stretch gap-2 w-full max-w-[180px]">
              ...
              <button
                ...
                className="px-3 py-2.5 rounded-xl border-[1.5px] border-[#009882] bg-white text-[#007f6d] hover:bg-[#009882] hover:text-white hover:border-[#009882] text-[11px] font-extrabold transition-all active:scale-95 whitespace-nowrap flex items-center justify-center gap-1.5 group/cmp">
        ...
        <div className="px-5 lg:px-6 py-2.5 border-t border-[#f0f1f4] flex items-center gap-2 flex-wrap bg-white">
          ...
          <span className="px-2.5 py-1 bg-[#e8f4fd] text-[#0172cb] text-[10px] font-extrabold rounded-full uppercase tracking-wider">{flight.cabin}</span>
          {flight.eco && (
          ...
          {flight.seats != null && (
            <span className={`ml-auto text-[11px] font-bold flex items-center gap-1 ${seatsTight ? 'text-[#d92d20]' : 'text-[#697d95]'}`}>
